...
namespace CBSMonitoring.Services
{
    public class ConcreteFormFactory : FormFactory
    {
        ...
        public override async Task<Result<Type[]>> GetMonitoringForm(string formNumber, int argCount)           
           {
            
            var response = formNumber switch
            {
            ...
            {
                return await Result<Type[]>.FailAsync(StatusCodes.Status400BadRequest, $"Номер формы {formNumber} не поддерживается!");
            }

            return await Result<Type[]>.SuccessAsync(response);
